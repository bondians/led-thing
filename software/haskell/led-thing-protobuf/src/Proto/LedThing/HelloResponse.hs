...
module Proto.LedThing.HelloResponse (HelloResponse(..)) where
import Prelude ((+), (/))
...
data HelloResponse = HelloResponse{request_id :: !(P'.Maybe P'.Word32), status :: !(P'.Maybe LedThing.ResultStatus),
                                   status_message :: !(P'.Maybe P'.Utf8)}
                   deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

instance P'.Mergeable HelloResponse where
  mergeAppend (HelloResponse x'1 x'2 x'3) (HelloResponse y'1 y'2 y'3)
   = HelloResponse (P'.mergeAppend x'1 y'1) (P'.mergeAppend x'2 y'2) (P'.mergeAppend x'3 y'3)

instance P'.Default HelloResponse where
  defaultValue = HelloResponse P'.defaultValue P'.defaultValue P'.defaultValue

instance P'.Wire HelloResponse where
  wireSize ft' self'@(HelloResponse x'1 x'2 x'3)
   = case ft' of
       10 -> calc'Size
  ...
  wirePut ft' self'@(HelloResponse x'1 x'2 x'3)
   = case ft' of
       10 -> put'Fields
       11 -> do
